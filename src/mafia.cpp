...
void* Mafia::update(const sf::RenderWindow& fen)
{
    ...
    if (_elapsed_time > _shot_delay) {
        _elapsed_time = 0.0f;
        return new Projectile("media/munition.png", _sprite.GetPosition().x, _sprite.GetPosition().y, Projectile::enemy, MAFIA_SHOT_SPEED, sf::Randomizer::Random(- (int) MAFIA_SHOT_SPEED, (int) MAFIA_SHOT_SPEED));
    }
    return NULL;
}
